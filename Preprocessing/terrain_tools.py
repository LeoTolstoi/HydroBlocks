...
def calculate_area(r):
    ...
    r.dy = calculate_distance(
        lats[0:-1, 0:-1], lats[1:, 1:], (lons[0:-1, 0:-1] + lons[1:, 1:]) / 2,
...
def cluster_data(X, nc):
    ...
    minsamples = int(
        (10**5) / 2)  # Taking very long per tile for large catchments
...
def alternative_cluster_data(X, nc):
    ...
    if nc > 1:
        ...
        model = sklearn.cluster.SpectralClustering(n_clusters=nc,
...
def compute_cluster_parameters(Xd, maxnc=1000):
    ...
    while flag == False:
        ...
        for var in Xd:
            ...
            tmp = normalize_variable(np.copy(Xd[var]['d']), Xd[var]['min'],
                                     Xd[var]['max'])
...
def compute_basin_delineation_nbasins(dem, mask, res, nbasins):
    ...
    channels = ttf.calculate_channels(area, channel_threshold, max_threshold,
                                      fdir)
    min_basins = ttf.delineate_basins(channels, mask, fdir)
    min_nbasins = np.unique(min_basins)[1::].size
    #print min_nbasins
    #Min iteration
    channels = ttf.calculate_channels(area, channel_threshold, min_threshold,
                                      fdir)
    ...
    for i in xrange(10):
        ...
        channels = ttf.calculate_channels(area, channel_threshold, np.exp(c),
                                          fdir)
        ...
        if nbasins < c_nbasins:
            ...
            channels = ttf.calculate_channels(area, channel_threshold,
                                              min_threshold, fdir)
            max_basins = ttf.delineate_basins(channels, mask, fdir)
            max_nbasins = np.unique(max_basins)[1::].size
        else:
...
def define_hrus(basins, dem, channels):
    ...
    for basin in ubasins:
        ...
        for ibin in xrange(nbins):
            smask = (basins
                     == basin) & (dem >= bins[ibin]) & (dem < bins[ibin + 1])
...
def calculate_basin_properties(basins, res, latitude, longitude, fdir):
    ...
    nb = np.max(basins)
    (ah, lath, lonh, hid,
     nid) = ttf.calculate_basin_properties(basins, res, nb, fdir, latitude,
                                           longitude)
    properties = {
        'area': ah,
        'latitude': lath,
...
def calculate_basin_properties_updated(basins, res, cvs, vars):
    ...
    for uh in masks.keys():
        ...
        for var in vars:
            #print(masks[uh])
            tmp[var] = cvs[var][masks[uh][:, 0],
                                masks[uh][:, 1]]  #imin:imax+1,jmin:jmax+1]
...
def calculate_hillslope_properties_updated(hillslopes, dem, res, latitude,
                                           ...
                                           aspect, tas, prec, cdir, uhrt, uhst,
                                           lt_uvt, ul_mask):
    ...
    #Initialize properties dictionary
    vars = [
        'latitude', 'longitude', 'dem', 'aspect', 'tas', 'prec', 'slope',
        'width_intercept', 'width_slope', 'length', 'area', 'd2c_array',
        'position_array', 'width_array', 'relief', 'x_aspect', 'y_aspect',
        'hid', 'relief_a', 'relief_b', 'uhrt', 'uhst', 'lt_uvt', 'ul_mask'
    ]
    properties = {}
    for var in vars:
        properties[var] = []
    ...
    for uh in masks.keys():
        ...
        if nc > 1:
            tmp = np.sort(sd2c[m])
            bin_edges = tmp[np.arange(
                0, tmp.size, np.int(np.ceil(float(tmp.size) / (nc + 1))))]
  ...
  for i in xrange(data['d2c'].size):
   ...
   length.append(ld+lu)'''
        ...
        if d2c.size == 3:
            ...
            if d2c[1:-1].size > 10:
                try:
                    fr, pcov = scipy.optimize.curve_fit(frelief,
                                                        position[1:-1],
                                                        d2c[1:-1],
                                                        bounds=([1.0, 1.0],
                                                                [5.0, 5.0]))
        ...
        tmp = {
            'latitude': latitude[imin:imax + 1, jmin:jmax + 1],
            'longitude': longitude[imin:imax + 1, jmin:jmax + 1],
            'dem': dem[imin:imax + 1, jmin:jmax + 1],
...
def calculate_hillslope_properties(hillslopes, dem, basins, res, latitude,
                                   longitude, depth2channel, slope, aspect,
                                   cplan, cprof, channels, tas, prec):

    nh = np.max(hillslopes) + 1
    (eh, ah, bh, lath, lonh, erange, hid, d2c, slope, haspect, hcplan, hcprof,
     hmaxd2c, hmind2c, htwidth, hbwidth, htas,
     hprec) = ttf.calculate_hillslope_properties(hillslopes, dem, basins, res,
                                                 nh, latitude, longitude,
                                                 depth2channel, slope, aspect,
                                                 cplan, cprof, channels, tas,
                                                 prec)
    properties = {
        'elevation': eh,
        'area': ah,
...
def create_tiles_kmeans(basins, covariates, ntiles):
    ...
    hrus = np.empty(covariates[covariates.keys()[0]]['data'].shape).astype(
        np.int32)
    hrus[:] = -9999
    ...
    for ib in ub:
        ...
        clf = sklearn.cluster.MiniBatchKMeans(ntiles,
                                              random_state=1,
                                              init=init,
    ...
    for ih in uh:
        ...
        for i in xrange(H.size):
            ...
            if H[coords] > 0:
                ...
                for var in covariates:
                    ...
                    clusters[icluster]['bounds'][var] = [
                        edges[key][coords[key]], edges[key][coords[key] + 1]
                    ]
            Hfl.next()
        ...
        for cid in clusters.keys():
            for id in covariates.keys():
                if covariates.keys().index(id) == 0:
                    string = "(covariates['%s']['data'] >= clusters[%d]['bounds']['%s'][0]) & (covariates['%s']['data'] <= clusters[%d]['bounds']['%s'][1]) & mask" % (
                        id, cid, id, id, cid, id)
                else:
                    string = string + " & (covariates['%s']['data'] >= clusters[%d]['bounds']['%s'][0]) & (covariates['%s']['data'] <= clusters[%d]['bounds']['%s'][1]) & mask" % (
                        id, cid, id, id, cid, id)
...
def create_hillslope_tiles(hillslopes, depth2channel, nbins, bins):
    ...
    for ih in uh:
        ...
        (hist,
         bins) = np.histogram(tmp,
                              bins=nbins[ih - 1])  #Change to predefined bins
        for ibin in xrange(nbins[ih - 1]):
            ...
            smask = mask & (depth2channel >= bins[ibin]) & (depth2channel
                                                            <= bins[ibin + 1])
...
def create_hillslope_tiles_updated(hillslopes, depth2channel, hillslopes_full,
                                   hp_in, hp):
    ...
    for ih in uh:
        ...
        for ibin in xrange(nbins):
            if ibin == 0:
                ...
                smask = mask & (new_hand > nhand[ibin]) & (new_hand
                                                           <= nhand[ibin + 1])
            clusters[smask] = ibin + 1
...
def create_basin_tiles(basin_clusters, hand, input_dem, basins, dh):
    ...
    for ubc in ubcs:
        ...
        bin_edges = np.linspace(7**(1.0 / float(pedges)),
                                data_max**(1.0 / float(pedges)),
                                nbins + 1)**pedges
        ...
        for i in range(bin_edges.size - 1):
            ...
            if np.sum(m2) > 0:
                ...
                for j in range(dem_edges.size - 1):
                    if j == 0:
                        m3 = m2 & (input_dem >= dem_edges[j]) & (
                            input_dem <= dem_edges[j + 1])
                    else:
                        m3 = m2 & (input_dem > dem_edges[j]) & (
                            input_dem <= dem_edges[j + 1])
...
def create_hrus_hydroblocks(hillslopes, htiles, channels, covariates,
                            nclusters):
    ...
    for uh in uhs:
        ...
        for ut in uts:
            ...
            for var in covariates:
                ...
                if var in ['lats', 'lons', 'basins'
                           ]:  # Normalize lat lon at the tile level -- Noemi
                    ccp[var] = {
                        'd': tmp,
                        't': covariates[var]['t'],
            ...
            for var in covariates:
                ...
                tmp = covariates[var]['w'] * normalize_variable(
                    covariates[var]['d'][mt], covariates[var]['min'],
                    covariates[var]['max'])
                mask_out = (
                    (np.isnan(tmp) | np.isinf(tmp)) | (tmp == -9999))  # Noemi
                #print(tmp.shape,flush=True)
                if sum(mask_out) > 0:
                    exit('%s has -9999 or invalid entries on the clustering' %
                         var)
...
def create_hrus(hillslopes, htiles, covariates, nclusters, flag, maxnc, cdir):
    ...
    #return hrus
    #Write out output
    pickle.dump(hrus, open('%s/hrus.pck' % cdir, 'wb'),
                pickle.HIGHEST_PROTOCOL)

    return


def calculate_hru_properties(hillslopes, tiles, channels, res, nhillslopes,
                             hrus, depth2channel, slope, basins, cdir):

    tmp = np.unique(hrus)
    tmp = tmp[tmp != -9999]
    nhru = tmp.size
    (wb, wt, l, hru_position, hid, tid, hru, hru_area, hru_dem,
     hru_slope) = ttf.calculate_hru_properties(hillslopes, tiles, channels,
                                               basins, nhru, res, nhillslopes,
                                               hrus, depth2channel, slope)
...
def calculate_hru_properties_updated(hillslopes, tiles, res, hrus,
                                     depth2channel, slope, hp, cdir, nhand):
    ...
    hru_properties = {}
    vars = [
        'hillslope_id', 'tile_id', 'hru', 'area', 'hillslope_slope',
        'hand_ecdf', 'hand_bedges'
    ]
    for var in vars:
        hru_properties[var] = []
    for hru in masks:
    ...
    vars = [
        'hillslope_length', 'hillslope_hand', 'hillslope_position',
        'hillslope_width', 'hillslope_frac', 'soil_depth', 'depth_to_bedrock'
    ]
    for var in vars:
    ...
    for ih in xrange(hp['hid'].size):
        ...
        p0 = hp['relief_p0'][ih]
        p1 = hp['relief_p1'][ih]
        length = hp['length'][ih] * (frelief_inv(nrelief[1:], p0, p1) -
                                     frelief_inv(nrelief[0:-1], p0, p1))
        ...
        hru_properties['frac'][
            m] = hp['frac'][ih] * hru_properties['hillslope_frac'][m]
        #print ih,tids.size
        ...
        if hp['ul_mask'][ih] >= 1.5:  #LOWLAND
            ...
            depth_to_bedrock = (soil_thickness +
                                sedimentary_thickness) * np.ones(tids.size)
...
def cluster_hillslopes(hillslopes, covariates, hp_in, nclusters, ws):
    ...
    for var in covariates:
        ...
        tmp = np.copy(otmp)
        #Normalize and apply weight
        tmp = covariates[var]['w'] * (tmp - np.min(tmp)) / (np.max(tmp) -
                                                            np.min(tmp))
        #Convert to percentiles
        #argsort = np.argsort(tmp)
        #tmp[argsort] = np.linspace(0,1,tmp.size)
    ...
    hillslopes_clusters = ttf.assign_clusters_to_hillslopes(
        hillslopes, clusters)
...
def cluster_hillslopes_updated(hillslopes, covariates, hp_in, nclusters, ws,
                               dh, max_nbands, min_nbands):
    ...
    for var in covariates:
        ...
        #Normalize and apply weight
        tmp = covariates[var]['w'] * normalize_variable(
            tmp, covariates[var]['min'], covariates[var]['max'])
    ...
    for cluster in uclusters:
        ...
        frac = hp_in['area'][m] / np.sum(hp_in['area'][m])
        for var in hp_in:
            if var in ['position_array', 'width_array', 'd2c_array', 'hid']:
                continue
            if var not in hp_out: hp_out[var] = []
            hp_out[var].append(np.sum(frac * hp_in[var][m]))
        #Calculate the fraction
  ...
  w = np.array(w)'''
        ...
        if len(hp_in['position_array'][mc].shape) > 1:
            w = 1 + hp_in['position_array'][
                mc, :] * hp_in['width_slope'][mc][:, np.newaxis]
        else:
        ...
        #Fit line to width
        try:
            fw, pcov = scipy.optimize.curve_fit(fwidth,
                                                p,
                                                w,
                                                bounds=([
                                                    -0.99,
...
def cluster_basins_updated(basins, covariates, hp_in, nclusters):
    ...
    for var in covariates:
        ...
        tmp = np.copy(otmp)
        #Normalize and apply weight
        tmp = normalize_variable(tmp, covariates[var]['min'],
                                 covariates[var]['max'])
        X.append(tmp)
...
def curate_hru_properties(hru_properties, hp):
    ...
    for hid in hp['hid']:
        ...
        #redo the length
        (d2c, idx) = np.unique(hru_properties['depth2channel'][m],
                               return_inverse=True)
        ...
        for tid in np.unique(tids):
            m1 = (tids == tid)
            f1.append(
                np.sum(hru_properties['area'][m][m1] /
                       np.sum(hru_properties['area'][m])))
        f1 = np.array(f1)
        ...
        for it in xrange(utids.size):
            m1 = tids == utids[it]
            f = hru_properties['area'][m][m1] / np.sum(
                hru_properties['area'][m][m1])
