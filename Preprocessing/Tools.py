...
def Create_LHS_parameter_set(nsamples):
    ...
    np.savetxt('LHS/LHS_sampling.txt',
               param_values,
               delimiter=' ',
               header=" ".join(vars))
...
def update_output(output, NOAH, TOPMODEL, date):
    ...
    output['variables']['lh'].append(np.copy(NOAH.fcev + NOAH.fgev +
                                             NOAH.fctr))  #W/m2
    output['variables']['swnet'].append(np.copy(NOAH.fsa))  #W/m2
    output['variables']['lwnet'].append(np.copy(NOAH.fira))  #W/m2
    #Water balance
    output['variables']['et'].append(
        np.copy(NOAH.ecan + NOAH.etran + NOAH.esoil))  #mm/s
    imax = TOPMODEL.outlet_hsu
    qexcess = np.copy(NOAH.runsb)
    qexcess[imax] = qexcess[imax] + 10**3 * (TOPMODEL.dx[imax] *
                                             (TOPMODEL.qout[imax]) /
                                             np.sum(TOPMODEL.area))
    ...
    output['variables']['sstorage'].append(
        np.copy(10**3 * TOPMODEL.storage_surface))  #mm
    output['variables']['sndpth'].append(np.copy(10**3 * NOAH.sndpth))  #mm
    output['variables']['swe'].append(np.copy(NOAH.swe))  #mm
    #Fluxes
    output['variables']['qout_subsurface'].append(np.copy(
        TOPMODEL.qout))  #m2/s
    output['variables']['qout_surface'].append(np.copy(
        TOPMODEL.qout_surface))  #m2/s
...
def run_model(parameters, cdir, idate, fdate, ncores, wbd):
    ...
    info[
        'MPTABLE'] = 'Model/pyNoahMP/data/MPTABLE.TBL'  #'pyNoahMP/data/MPTABLE.TBL'
    ...
    while date <= fdate:
        if (date.hour == 0) and (date.day == 1):
            print date, np.sum(
                TOPMODEL.pct * TOPMODEL.si
            ), time.time() - tic, et, prcp, 'q:%f' % q, np.sum(
                TOPMODEL.pct * NOAH.smcwtd), np.sum(
                    TOPMODEL.pct * NOAH.zwt), 'WB ERR:%f' % np.sum(
                        TOPMODEL.pct * errwat
                    ), 'etran:%f' % etran, 'ecan:%f' % ecan, 'esoil:%f' % esoil
        ...
        smw = 1000.0 * ((NOAH.smcmax * np.abs(NOAH.zwt) - TOPMODEL.si) +
                        (NOAH.smcmax * (100 - np.abs(NOAH.zwt))))
        beg_wb = np.copy(NOAH.canliq + NOAH.canice + NOAH.swe + NOAH.wa +
                         smw)  #TOPMODEL.si*1000)
        ...
        smw = 1000.0 * ((NOAH.smcmax * np.abs(NOAH.zwt) - TOPMODEL.si) +
                        (NOAH.smcmax * (100.0 - np.abs(NOAH.zwt))))
        end_wb = np.copy(NOAH.canliq + NOAH.canice + NOAH.swe + NOAH.wa +
                         smw)  # + TOPMODEL.si*1000)

        #Update the water balance error
        tmp = np.copy(end_wb - beg_wb - NOAH.dt *
                      (NOAH.prcp - NOAH.ecan - NOAH.etran - NOAH.esoil -
                       NOAH.runsf - NOAH.runsb) - 1000 * dzwt0)
        errwat += tmp  # - tmp
        q = q + 3600. * np.sum(TOPMODEL.pct * NOAH.runsb)
        et = et + 3600 * np.sum(TOPMODEL.pct *
    ...
    pickle.dump(output, open('%s/output/output.pck' % cdir, 'wb'),
...
def Update_Input(NOAH, TOPMODEL, date, meteorology, i):
    ...
    NOAH.yearlen = (datetime.datetime(date.year + 1, 1, 1, 0) -
    ...
    estar = 611.0 * np.exp(17.27 * ((NOAH.t_ml - 273.15) /
                                    (NOAH.t_ml[:] - 36.0)))
...
def create_netcdf_file(file_netcdf, output, nens, input, cdir):
    ...
    times[:] = nc.date2num(np.array(dates),
                           units=times.units,
    ...
    hsus = []
    for value in xrange(len(output['misc']['pct'])):
        hsus.append(value)
    hsu[:] = np.array(hsus)
    ...
    os.system(
        'gdalwarp -tr %.16f %.16f -dstnodata %.16f -t_srs EPSG:4326 -s_srs EPSG:102039 -te %.16f %.16f %.16f %.16f %s %s >> %s 2>&1'
...
def update_netcdf(cdir, iens, nens, parameters):
    ...
    for var in output['variables']:
        ...
        for itime in xrange(output['variables'][var].shape[0] / 24):
            data.append(
                np.mean(output['variables'][var][24 * itime:24 * (itime + 1)],
                        axis=0))
        data = np.array(data)
