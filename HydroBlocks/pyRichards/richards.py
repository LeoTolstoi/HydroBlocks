...
class richards:
    ...
    def calculate_transmissivity(self, psi, ztop, zbot):
        ...
        with np.errstate(invalid='ignore', divide='ignore'):
            K_x = Ksat_x * np.true_divide(
                psi, self.satpsi)**(-2 - np.true_divide(3., self.b))
    ...
    def calculate_divergence_dense(self, h, T):
        ...
        dx = self.dx
        area = self.area
        That = np.true_divide((2 * T[:, np.newaxis] * T[np.newaxis, :]),
                              (T[:, np.newaxis] + T[np.newaxis, :]))
        That[~np.isfinite(That)] = np.nan
        # [mm/s] = [mm/m]*[m/s]*[m]/[m]*[m]*[m]/[m2]
        calc_div = -1000.0 * That * np.true_divide(dh, dx) * np.true_divide(
            w, area)  # mm/s
    ...
    def calculate_divergence_sparse(self, h, T):
        ...
        print(1.0 / self.area)
        print(
            That.multiply(dh).multiply(self.width).multiply(
                1.0 / self.area).count_nonzero)
        print(
            That.multiply(dh).multiply(self.width).multiply(
                1.0 / self.area).multiply(dx.power(-1)).count_nonzero)
        return -That.multiply(dh).multiply(self.width).multiply(
            1.0 / self.area).multiply(dx.power(-1)).multiply(1000)  # mm/s
    ...
    def update_numba(self):
        ...
        dx = self.dx
        area = self.area
        self.hdiv[:] = update_workhorse(theta, dz, hdiv, thetar, thetas, b,
                                        satpsi, m, ksat, hand, w, dx, area)

        return


@numba.jit(nopython=True, cache=True)
def update_workhorse(theta, dz, hdiv, thetar, thetas, b, satpsi, m, ksat, hand,
                     w, dx, area):
