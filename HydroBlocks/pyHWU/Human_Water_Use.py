...
class Human_Water_Use:
    ...
    def __init__(self, HB, info):
        ...
        if self.hwu_flag == True:
            if not any([self.hwu_gw_flag, self.hwu_sf_flag]):
                exit(
                    'Error: All water supply flags are off - at least one water source is needed'
                )
        ...
        demands = [
            self.hwu_agric_flag, self.hwu_domest_flag, self.hwu_indust_flag,
            self.hwu_lstock_flag
        ]
        if self.hwu_flag == True:
            if not any(demands):
        ...
        self.beta = HB.input_fp.groups['parameters'].variables[
            'slope'][:]  #Noemi
        m = HB.input_fp.groups['parameters'].variables[
            'm'][:]  #Depth to bedrock #Noemi
        ...
        if self.hwu_gw_flag == True:
            ...
            self.well_depth = np.maximum(-1.0 * m * 0.8,
                                         NOAH.zsoil[:, -1] * 0.8)
            self.well_layer = np.argmin(
                abs(NOAH.zsoil - self.well_depth[:, np.newaxis]))
            ...
            self.mask_gw = np.array([
                False if i <= 10.0**-6 or j >= 50. else True
                for i, j in zip(NOAH.satdk0, NOAH.clay_pct)
            ])
        ...
        if self.hwu_sf_flag == True:
            ...
            cond1 = (self.beta
                     < 0.01)  #( self.sti >= 10.0 ) & ( self.beta < 0.01)
            #cond1 = ( self.sti >= 10.0 )
            # surface water from HRUs with land cover as water or wetlands
            cond2 = np.array([
                True
                if x in self.water_use_land_cover["surface_water"] else False
                for x in NOAH.vegtyp
            ])
            self.mask_sf = np.array([
                True if i == True or j == True else False
                for i, j in zip(cond1, cond2)
            ])
        ...
        if self.hwu_agric_flag == True:
            ...
            self.mask_agric = [
                True
                if x in self.water_use_land_cover['agriculture'] else False
                for x in NOAH.vegtyp
            ]
            ...
            self.irrig_land = HB.input_fp.groups['parameters'].variables[
                'irrig_land'][:]
            self.mask_irrig = self.mask_agric & (self.irrig_land > 0.0)
            print('Irrig Percentage:',
                  np.sum(self.mask_irrig) / float(np.sum(self.mask_agric)))

            # Crop calendar
            self.st_gscal = np.asarray(HB.input_fp.groups['parameters'].
                                       variables['start_growing_season'][:],
                                       dtype=np.int)
            self.en_gscal = np.asarray(HB.input_fp.groups['parameters'].
                                       variables['end_growing_season'][:],
                                       dtype=np.int)
        ...
        if self.hwu_indust_flag == True:
            ...
            self.mask_indust = [
                True if x in self.water_use_land_cover['industrial'] else False
                for x in NOAH.vegtyp
            ]
        ...
        if self.hwu_domest_flag == True:
            ...
            self.mask_domest = [
                True if x in self.water_use_land_cover['domestic'] else False
                for x in NOAH.vegtyp
            ]
        ...
        if self.hwu_lstock_flag == True:
            ...
            self.mask_lstock = [
                True if x in self.water_use_land_cover['livestock'] else False
                for x in NOAH.vegtyp
            ]
    ...
    def initialize_allocation(self, HB):
        ...
        if self.hwu_flag == True:
            ...
            # 1. HRU distances
            # Centroid Distances
            self.ctrd_lats = HB.input_fp.groups['parameters'].variables[
                'centroid_lats'][:]
            self.ctrd_lons = HB.input_fp.groups['parameters'].variables[
                'centroid_lons'][:]
            self.hrus_centroid_distances = mgmt_funcs.hrus_centroid_distance(
                self.ctrd_lats, self.ctrd_lons)  # km
            # Minimum Distance between a HRU centroid and it's neighboors boundary
            self.hru_min_dist = HB.input_fp.groups['parameters'].variables[
                'hru_min_dist'][:]
            self.hrus_boundary_distances = np.copy(self.hru_min_dist)  # km
            # Assign Distaces to Surface of Grounwater supply
            print("HRU's GW distance - mean:%f and std:%f" %
                  (np.mean(self.hrus_centroid_distances[
                      self.hrus_centroid_distances > 0.]),
                   np.std(self.hrus_centroid_distances[
                       self.hrus_centroid_distances > 0.])))
            print("HRU's SF distance - mean:%f and std:%f" %
                  (np.mean(self.hrus_boundary_distances[
                      self.hrus_boundary_distances > 0.]),
                   np.std(self.hrus_boundary_distances[
                       self.hrus_boundary_distances > 0.])))
            ...
            self.hrus_slopes = mgmt_funcs.hrus_slope(
                self.dem, self.hrus_centroid_distances)  # m/m
            ...
            if self.hwu_sf_flag == True:
                ...
                m = np.copy(np.invert(self.mask_sf))
                self.ratio_sf[:, m, :] = 0.0
                m = np.copy(self.hrus_boundary_distances > self.sf_dist_lim)
                self.ratio_sf[:, m] = 0.0
                m = np.copy(self.hrus_slopes < self.sf_slope_lim)
                self.ratio_sf[:, m] = 0.0
            ...
            if self.hwu_gw_flag == True:
                ...
                # Groundwater Ratio
                self.ratio_gw = np.ones(
                    (self.nwuse_index, ncells, ncells)) * 1.0
                ...
                # Update Ratio for Supply
                m = np.copy(np.invert(self.mask_gw))
                self.ratio_gw[:, m, :] = 0.0
                m = np.copy(self.hrus_centroid_distances > self.gw_dist_lim)
                self.ratio_gw[:, m] = 0.0
                m = np.copy(self.hrus_slopes < self.gw_slope_lim)
                self.ratio_gw[:, m] = 0.0
                # sum over the diff water sectors, then over the demand nodes
    ...
    '''
                ...
                # Groundwater Cost
                self.cost_gw = np.ones(
                    (self.nwuse_index, ncells, ncells)) * 2.0
            ...
            if self.hwu_optimal_allocation_flag == True:
                ...
                # Define Model
                self.ntwkm = wr_Model(start="2017-01-01",
                                      end="2017-01-01",
                                      timestep=1,
                                      solver='glpk')
                ...
                if self.hwu_sf_flag == True and any(sf_valid_links):
                    ...
                    if self.hwu_gw_flag == True and any(gw_valid_links):
                        self.valid_links = np.concatenate(
                            [sf_valid_links, gw_valid_links])
                ...
                if any(self.valid_links):
                    ...
                    nodes_names = [
                        str(self.nodes_list[i][0].name) for i in range(n_nodes)
                    ]
                    nodes_types = [
                        str(self.nodes_list[i][0].__class__.__name__)
                    ...
                    if self.hwu_agric_flag == True:
                        ...
                        for i in a_nodes_names:
                            self.a_nodes_position.append(nodes_names.index(i))
                    if self.hwu_domest_flag == True:
                        self.d_nodes_position = []
                        for i in d_nodes_names:
                            self.d_nodes_position.append(nodes_names.index(i))
                    if self.hwu_indust_flag == True:
                        self.i_nodes_position = []
                        for i in i_nodes_names:
                            self.i_nodes_position.append(nodes_names.index(i))
                    if self.hwu_lstock_flag == True:
                        self.l_nodes_position = []
                        for i in l_nodes_names:
                            self.l_nodes_position.append(nodes_names.index(i))

                    if self.hwu_sf_flag == True:
                        self.s_nodes_position = []
                        for i in s_nodes_names:
                            self.s_nodes_position.append(nodes_names.index(i))
                        self.s_links_position = []
                        for i in s_links_names:
                            self.s_links_position.append(nodes_names.index(i))

                    if self.hwu_gw_flag == True:
                        self.g_nodes_position = []
                        for i in g_nodes_names:
                            self.g_nodes_position.append(nodes_names.index(i))
                        self.g_links_position = []
                        for i in g_links_names:
                            self.g_links_position.append(nodes_names.index(i))
    ...
    def Calc_Human_Water_Demand_Supply(self, HB, date):
        ...
        if (date.hour * 3600.0) % self.dta == 0.0:
            ...
            if self.hwu_flag == True:
                ...
                if self.hwu_indust_flag == True:
                    self.deficit_indust = np.copy(
                        self.demand_indust) * self.dta * area
                if self.hwu_domest_flag == True:
                    self.deficit_domest = np.copy(
                        self.demand_domest) * self.dta * area
                if self.hwu_lstock_flag == True:
                    self.deficit_lstock = np.copy(
                        self.demand_lstock) * self.dta * area
            ...
            if self.hwu_flag == True:
                ...
                # Convert m to m3
                if self.hwu_sf_flag == True:
                    self.supply_sf = self.supply_sf * area
                if self.hwu_gw_flag == True:
                    self.supply_gw = self.supply_gw * area
            ...
            if self.hwu_agric_flag == True:
                self.deficit_agric = self.deficit_agric / (self.dta / area)
            ...
            if self.hwu_flag == True:
                ...
                with np.errstate(divide='ignore'):
                    # Convert from m3 to m/s
                    if self.hwu_indust_flag == True:
                        self.deficit_indust = self.deficit_indust / (self.dta /
                                                                     area)
                    if self.hwu_domest_flag == True:
                        self.deficit_domest = self.deficit_domest / (self.dta /
                                                                     area)
                    if self.hwu_lstock_flag == True:
                        self.deficit_lstock = self.deficit_lstock / (self.dta /
                                                                     area)

                    # Convert from m3 to m/tstep
                    if self.hwu_agric_flag == True:
                        self.alloc_agric = self.alloc_agric / (area / self.ntt)
                    if self.hwu_gw_flag == True:
                        self.alloc_gw = self.alloc_gw / (area / self.ntt)
                    if self.hwu_sf_flag == True:
                        self.alloc_sf = self.alloc_sf / area  #/self.ntt #Abstract sf water just on dta
    ...
    def Calc_Water_Supply(self, HB):
        ...
        if self.hwu_flag == True:
            ...
            if self.hwu_gw_flag == True:
                ...
                dsm = NOAH.sh2o - NOAH.smcref[:, np.
                                              newaxis]  #moisture content between
                dsm[dsm < 0] = 0
                self.supply_gw = np.array([
                    np.sum(dsm[i, :self.well_layer + 1] *
                           sldpth[i, :self.well_layer + 1])
                    for i in range(ncells)
                ])
            ...
            if self.hwu_sf_flag == True:
                ...
                self.supply_sf = self.supply_sf * (HB.dt / 1000.
                                                   )  # from mm/s to m
        ...
        if self.hwu_flag == True:
            ...
            if self.hwu_gw_flag == True:
                ...
                if HB.subsurface_module == 'dtopmodel':
                    ...
                    dsm[dsm < 0] = 0
                    volume_avail = np.array([
                        dsm[i, :] * sldpth[i, :] for i in range(ncells)
                    ])  #m per layer
                    ...
                    m = (delta_dhiv > 0)
                    HB.richards.hdiv[m] = (HB.richards.hdiv -
                                           delta_dhiv)[m]  # mm/s
    ...
    def Human_Water_Irrigation(self, HB, date):
        if self.hwu_flag == True:
            if self.hwu_agric_flag == True:  # and (date.hour*3600)%self.dta == 0:
                ...
                self.irrigation = np.copy(self.alloc_agric) * (
                    1000.0 / HB.noahmp.dt)  #from m/tstep to mm/s
                m = (self.mask_irrig == True)
                #print self.irrigation
                #print self.irrigation[m]
                #print 'irrig',np.sum(self.irrigation)*(HB.noahmp.dt/1000.0)
                HB.noahmp.prcp[m] = (HB.noahmp.prcp +
                                     self.irrigation)[m]  #mm/s
    ...
    def Optimal_Water_Allocation(self, NOAH):
        ...
        if any(self.valid_links.flatten()):
            ...
            if self.hwu_agric_flag == True:
                ...
                for p, d in zip(self.a_nodes_position,
                                a_deficit[self.mask_irrig]):
                    self.nodes_list[p][0].max_flow = d

            if self.hwu_indust_flag == True:
                i_deficit = np.copy(self.deficit_indust)
                for p, d in zip(self.i_nodes_position,
                                i_deficit[self.mask_indust]):
                    self.nodes_list[p][0].max_flow = d

            if self.hwu_domest_flag == True:
                d_deficit = np.copy(self.deficit_domest)
                for p, d in zip(self.d_nodes_position,
                                d_deficit[self.mask_domest]):
                    self.nodes_list[p][0].max_flow = d

            if self.hwu_lstock_flag == True:
                l_deficit = np.copy(self.deficit_lstock)
                for p, d in zip(self.l_nodes_position,
                                l_deficit[self.mask_lstock]):
                    self.nodes_list[p][0].max_flow = d
            ...
            if self.hwu_sf_flag == True:
                ...
                for p, s in zip(self.s_nodes_position,
                                sf_supply[self.mask_sf]):
            ...
            if self.hwu_gw_flag == True:
                ...
                for p, s in zip(self.g_nodes_position,
                                gw_supply[self.mask_gw]):
                    self.nodes_list[p][0].max_flow = s
            ...
            if self.hwu_sf_flag == True:
                ...
                max_flow = np.copy(
                    np.transpose(
                        np.transpose(self.ratio_sf, (0, 2, 1)) * sf_supply,
                        (0, 2, 1))[m])
                #print max_flow
            ...
            if self.hwu_gw_flag == True:
                ...
                max_flow = np.copy(
                    np.transpose(
                        np.transpose(self.ratio_gw, (0, 2, 1)) * gw_supply,
                        (0, 2, 1))[m])
            ...
            if self.check_setup() == True:
                ...
                if self.hwu_agric_flag == True:
                    delta[:] = 0
                    for p, h in zip(self.a_nodes_position,
                                    np.where(self.mask_irrig)[0]):
                ...
                if self.hwu_indust_flag == True:
                    delta[:] = 0
                    for p, h in zip(self.i_nodes_position,
                                    np.where(self.mask_indust)[0]):
                        delta[h] = np.copy(self.nodes_list[p][0].flow[0])
                    self.deficit_indust = np.copy(i_deficit - delta)

                if self.hwu_domest_flag == True:
                    delta[:] = 0
                    for p, h in zip(self.d_nodes_position,
                                    np.where(self.mask_domest)[0]):
                        delta[h] = np.copy(self.nodes_list[p][0].flow[0])
                    self.deficit_domest = np.copy(d_deficit - delta)

                if self.hwu_lstock_flag == True:
                    delta[:] = 0
                    for p, h in zip(self.l_nodes_position,
                                    np.where(self.mask_lstock)[0]):
                        delta[h] = np.copy(self.nodes_list[p][0].flow[0])
                    self.deficit_lstockl = np.copy(l_deficit - delta)

                if self.hwu_sf_flag == True:
                    delta[:] = 0
                    for p, h in zip(self.s_nodes_position,
                                    np.where(self.mask_sf)[0]):
                        delta[h] = np.copy(self.nodes_list[p][0].flow[0])
                    #print 'delta sf',delta
                    self.alloc_sf = np.copy(delta)
                    pos = np.where((self.alloc_sf > 0.0)
                                   & (self.alloc_sf < 1e-10))[0]
                    #print pos
                    for p in pos:
                ...
                if self.hwu_gw_flag == True:
                    delta[:] = 0
                    for p, h in zip(self.g_nodes_position,
                                    np.where(self.mask_gw)[0]):
                        delta[h] = np.copy(self.nodes_list[p][0].flow[0])
                    #print 'delta gw',delta
                    self.alloc_gw = np.copy(delta)
                    pos = np.where((self.alloc_gw > 0.0)
                                   & (self.alloc_gw < 1e-10))[0]
                    for p in pos:
                ...
                if def2correct > 0:
                    if self.hwu_agric_flag == True:
                        ...
                        self.alloc_agric[
                            pos] = self.alloc_agric[pos] - def2correct
                        self.deficit_agric[
                            pos] = self.deficit_agric[pos] + def2correct
                    elif self.hwu_indust_flag == True:
                        pos = np.argmax(self.deficit_indust)
                        self.deficit_indust[
                            pos] = self.deficit_indust[pos] + def2correct
                    elif self.hwu_domest_flag == True:
                        pos = np.argmax(self.deficit_domest)
                        self.deficit_domest[
                            pos] = self.deficit_domest[pos] + def2correct
                    elif self.hwu_lstock_flag == True:
                        pos = np.argmax(self.deficit_lstock)
                        self.deficit_lstock[
                            pos] = self.deficit_lstock[pos] + def2correct
                    else:
  ...
  """
        ...
        if any(m):
            ...
            dsm[dsm < 0] = 0.0
            soil_demand[:] = np.array([
                np.sum(dsm[i, :nroot_zone_depht[i]] *
                       sldpth[i, :nroot_zone_depht[i]]) for i in range(ncells)
            ])
    ...
    def check_setup(self, ):
        ...
        total_gw_supply = 0.0
        if self.hwu_agric_flag == True:
            total_deficit += np.sum(self.deficit_agric[self.mask_irrig])
        if self.hwu_domest_flag == True:
            total_deficit += np.sum(self.deficit_domest[self.mask_domest])
        if self.hwu_indust_flag == True:
            total_deficit += np.sum(self.deficit_indust[self.mask_indust])
        if self.hwu_lstock_flag == True:
            total_deficit += np.sum(self.deficit_lstock[self.mask_lstock])
        if self.hwu_sf_flag == True:
            total_sf_supply = np.sum(self.supply_sf[self.mask_sf])
        if self.hwu_gw_flag == True:
            total_gw_supply = np.sum(self.supply_gw[self.mask_gw])

        if total_deficit != 0.0 and (total_sf_supply != 0.0
                                     or total_gw_supply != 0.0):
            out = True
        else:
            out = False
        return out
